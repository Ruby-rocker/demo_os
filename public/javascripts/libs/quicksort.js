...
Mootils.QuickSort = new Class({
  ...
  getFunc: function() {
    var func;
    if (this.sortBy && this.options.sortTypes && this.options.sortFunctions) {
      var idx = this.options.sortTypes.indexOf(this.sortBy);
      if ($type(this.options.sortFunctions[idx]) === 'function') func = this.options.sortFunctions[idx];
    }
    return func || this.getTextArray;
  },
  setSort: function() {
  ...
  getTextArray: function(arr) {
    ...
    arr.each(function(el, idx) {
      a.push([ el.get('text').toLowerCase().replace(/[^a-z0-9]/g, ''), idx ])
    }.bind(this));
    return a;
  },
